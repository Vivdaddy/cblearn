...
class LowRankSOE(BaseEstimator, TripletEmbeddingMixin):
    """ Low Rank Soft Ordinal Embedding (LowRankSOE).

        LowRankSOE minimizes the soft objective as a smooth relaxation of the triplet error as defined by 
        [1] with a regularizer to promote low-rank solutions. 
        ...
        This class restarts the optimization from multiple random initializations to increase the probability of
        good results in low-dimensional embeddings. This behaviour multiplies computation time when fitting
        the embeding but can be disabled by `LowRankSOE(n_components, n_init=1)`.
        ...
        Attributes:
            ...
            stress_: Final value of the LowRankSOE stress corresponding to the embedding.
        ...
        ((1000, 3), (15,))
        >>> estimator = LowRankSOE(n_components=2, random_state=seed)
        ...
        >>> estimator = LowRankSOE(n_components=2, backend="torch", random_state=seed)
        >>> embedding = estimator.fit_transform(triplets, n_objects=15)
    ...
    def fit(self, X: utils.Query, y: np.ndarray = None, init: np.ndarray = None,
        ...
        for init in inits:
            ...
            if self.verbose and not result.success:
                print(f"LowRankSOE's optimization failed: {result.message}.\n"
